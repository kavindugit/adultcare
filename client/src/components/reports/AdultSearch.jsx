...
export default function AdultSearch({ open, handleClose }) {
    ...
    const fetchAllAdultsData = async () => {
      try {
        const { data } = await axios.get(
          "http://localhost:4000/api/healthreport/adults"
        );
        if (data.success) {
  ...
  return (
    <Dialog
      ...
      slotProps={{
        paper: {
          style: { component: "form", onSubmit: handleSubmit },
        },
      }}
    >
      ...
      >
        <DialogContentText>Search Adult.</DialogContentText>
        ...
        {adultIdError && (
          <p style={{ color: "red", margin: 0 }}>Adult ID is required.</p>
        )}
