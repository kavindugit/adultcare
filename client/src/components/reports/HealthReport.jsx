...
const HealthReport = ({ padultId }) => {
  ...
  const [isNewDocNote, setIsNewDocNote] = useState(true);
  const [isNewCGNote, setIsNewCGNote] = useState(true);
  const [isNewNurNote, setIsNewNurNote] = useState(true);
  const [isNewPrescNote, setIsNewPrescNote] = useState(true);
  ...
  const fetchAdultInfo = async () => {
    setIsPrescription(false);
    try {
      const response = await axios.get(
        "http://localhost:4000/api/healthreport/adultInfo",
        {
      ...
      if (res.data.success) {
        {
          res.data.notes.map((note) => {
            if (note.noteType === "Doctor") {
              ...
              console.log(dialogContentDocRef.current.textContent? '' : '');
              setIsNewDocNote(false);
            } else if (note.noteType === "Nurse") {
              dialogContentNurseRef.current.textContent = note.note;
              console.log(dialogContentNurseRef.current.textContent? '' : '');
              setIsNewNurNote(false);
            } else if (note.noteType === "Caregiver") {
              dialogContentCGRef.current.textContent = note.note;
              console.log(dialogContentCGRef.current.textContent? '' : '');
              setIsNewCGNote(false);
            } else if (note.noteType === "Prescription") {
              dialogContentPresRef.current.textContent = note.note;
              console.log(dialogContentPresRef.current.textContent? '' : '');
              setIsNewPrescNote(false);
            }
          });
        }
      }
    ...
    try {
      ...
      if (isNewNote) {
        ...
        const response = await axios.put(
          "http://localhost:4000/api/healthreport/addNote",
          {
            note: noteContent,
        ...
        const response = await axios.post(
          "http://localhost:4000/api/healthreport/updateNote",
          {
            note: noteContent,
            noteType: isPrescription ? "Prescription" : appState.userData.role,
            adultId: appState.adultId,
      ...
      console.error(error);
      toast.error("Something went wrong. Chandana Please try again." + { error });
    }
  };
  ...
  const handleDelete = async (e) => {
    try {
      ...
      const response = await axios.delete(
        "http://localhost:4000/api/healthreport/deleteNote",
        {
          data: {
            noteType: isPrescription ? "Prescription" : appState.userData.role,
            adultId: appState.adultId,
  ...
  return (
    <div className="relative max-w-2xl mx-auto mt-9 p-10 bg-white rounded-lg shadow-lg border border-blue-300">
        ...
        <Fragment>
          ...
          >
            ...
            <DialogActions>
              ...
              <Button type="submit" onClick={() => setAction('save')}>{isNewNote ? " Add " : "Update"}</Button>
              {isNewNote ? (
                ""
              ) : (
      ...
      <Box
        component="section"
        className="relative w-full h-20 bg-blue-300 rounded-lg flex items-center justify-center"
        width="100%"
      >
